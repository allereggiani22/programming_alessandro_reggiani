...
- Print F in a quit aesthetical way
...
def score_m(s1,s2,d,matrix):
    ...
    for i in range(len(F[0][:-1])):     #initialize first row
        F[0][i+1]=F[0][i]-d
        traceback[0][i+1]="L"
    for i in range(len(F[:-1])):        #initialize first column
        F[i+1][0]=F[i][0]-d
        traceback[i+1][0] = "U"
    for i in range(1,N):
...
#alns will be printed reversed at the end
    ...
    while i>0 and j>0:
        if traceback[i][j]=="D":
            ...
            aln2+=s2[i-1]
            i-=1
    return aln1[::-1],aln2[::-1],F[len(s2)][len(s1)]
